...
describe("mCaptcha login example", function (this: ExtendDescribeThis<CustomThis>) {
  ...
  it("Test view and copy secret in settings page", async (browser) => {
    ...
    //  let emailText = await browser
    //    .click(this.settingsCopyBtnCssSelector!)
    //    .pause(20000)
    //    .click("#email")
    //    .keys([browser.Keys.CONTROL, "a", browser.Keys.BACK_SPACE])
    //    .keys([browser.Keys.CONTROL, "v"])
    //    .pause(2000)
    //    .getText("#email");
    //  expect(emailText).to.be.equal(secret);
